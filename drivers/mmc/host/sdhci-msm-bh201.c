				   ...
				   u8 * in_data, u32 datalen)
{
	...
	if (!data1) {
		DbgInfo("BHT MSG:gg read no memory\n");
		rc = -ENOMEM;
		goto out;
	}
														...
														u8 num)
{
    ...
    bool ret = FALSE;
		struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
		struct sdhci_msm_host *msm_host = pltfm_host->priv;
		ggc_platform_t *ggc = &msm_host->ggc;
				     ...
				     u8 * in_data, u32 datalen)
{
	...
	if (!data1) {
		DbgInfo("BHT MSG:gg write no memory\n");
		rc = -ENOMEM;
		goto out;
	}
...
bool gg_emulator_write(struct sdhci_host * host, u8 * data, u32 datalen)
{
	...
	if (1)
	{
		u32 i = 0;
		u32 reg;
		DbgInfo("BHT MSG:%s: dump config data\n", __FUNCTION__);
		for (i = 0; i < (datalen/sizeof(u32)); i++)
		{
			memcpy(&reg, data+i*sizeof(u32), sizeof(u32));
			DbgInfo("BHT MSG:\tggc_reg32[%03d]=0x%08x\n", i, reg);
		}
	}
...
bool gg_fix_output_tuning_phase(struct sdhci_host * host, int sela, int selb)
{
	...
	DbgInfo("BHT MSG:### %s - sela dll: %x, selb dll: %x \n", __FUNCTION__, sela,
		 selb);
...
bool _check_bus_mode(struct sdhci_host * host)
{
	...
	if (is_bus_mode_sdr104(host)) {
		ggc->cur_bus_mode = &msm_host->ggc.sdr104;
	} else {
		ggc->cur_bus_mode = &msm_host->ggc.sdr50;
	}
	return true;
}
...
void tx_selb_failed_history_update(struct sdhci_host *host, u32 val)
{
	...
	msm_host->ggc.cur_bus_mode->tx_selb_failed_history &= val;
}
...
void tx_selb_failed_history_reset(struct sdhci_host *host)
{
	...
	msm_host->ggc.sdr50.tx_selb_failed_history = BIT_PASS_MASK;
	msm_host->ggc.sdr104.tx_selb_failed_history = BIT_PASS_MASK;
}
...
u32 tx_selb_failed_history_get(struct sdhci_host *host)
{
	...
	return msm_host->ggc.cur_bus_mode->tx_selb_failed_history;
}
...
void tx_selb_failed_tb_update(struct sdhci_host *host, int sela, u32 val)
{
	...
	_check_bus_mode(host);
	msm_host->ggc.cur_bus_mode->tx_selb_tb[sela] &= val;
	DbgInfo(MODULE_SD_HOST, FEATURE_VENREG_TRACEW, NOT_TO_RAM, "%s %xh %xh\n", 
			__FUNCTION__, val, msm_host->ggc.cur_bus_mode->tx_selb_tb[sela]);
}
...
void tx_selb_failed_tb_reset(struct sdhci_host *host)
{
	...
	os_memset(&msm_host->ggc.sdr104.tx_selb_tb, 0xff, sizeof(msm_host->ggc.sdr104.tx_selb_tb));
	os_memset(&msm_host->ggc.sdr50.tx_selb_tb, 0xff, sizeof(msm_host->ggc.sdr50.tx_selb_tb));
}
...
u32 tx_selb_failed_tb_get(struct sdhci_host *host, int sela)
{
	...
	if (is_bus_mode_sdr104(host)) {
		...
		return msm_host->ggc.sdr104.tx_selb_tb[sela];
	} else {
		DbgInfo(MODULE_SD_HOST, FEATURE_VENREG_TRACEW, NOT_TO_RAM,
			"%s %xh\n", __FUNCTION__, g_tx_selb_tb_sdr50[sela]);
		return msm_host->ggc.sdr50.tx_selb_tb[sela];
	}
}
...
void all_selb_failed_tb_update(struct sdhci_host *host, int sela, u32 val)
{
	...
	_check_bus_mode(host);
	msm_host->ggc.cur_bus_mode->all_selb_tb[sela] &= val;
	DbgInfo(MODULE_SD_HOST, FEATURE_VENREG_TRACEW, NOT_TO_RAM,
			"%s %xh %xh\n", __FUNCTION__, val,
			msm_host->ggc.cur_bus_mode->all_selb_tb[sela]);
}
...
void all_selb_failed_tb_reset(struct sdhci_host *host)
{
	...
	os_memset(msm_host->ggc.sdr104.all_selb_tb, 0xff, sizeof(msm_host->ggc.sdr104.all_selb_tb));
	os_memset(msm_host->ggc.sdr50.all_selb_tb, 0xff, sizeof(msm_host->ggc.sdr50.all_selb_tb));
}
...
u32 all_selb_failed_tb_get(struct sdhci_host *host, int sela)
{
	...
	val = msm_host->ggc.cur_bus_mode->all_selb_tb[sela];
	DbgInfo(MODULE_SD_HOST, FEATURE_VENREG_TRACEW, NOT_TO_RAM,
			"%s %xh\n", __FUNCTION__, val);
					      ...
					      int tuning_cnt)
{
	...
	DbgInfo("BHT MSG:%s selb:%xh,tuning_cnt:%xh\n", __FUNCTION__, selb,
		 tuning_cnt);
...
int sdhci_bht_sdr50_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	...
	data_buf = kmalloc(size, GFP_KERNEL);
	if (!data_buf) {
		DbgInfo("BHT MSG:tuning no memory\n");
		rc = -ENOMEM;
		goto out;
	}
		    ...
		    u32 * rfb)
{
	...
	DbgInfo("BHT MSG:tgb:%xh, tgb len:%x\n", tgb, len_tb[tgb]);
	if (len_tb[tgb] < 6) {
		...
		if (0 == len_tb[lft1] || 0 == len_tb[lft2]) {
			...
			len_tb[lft1] = 0;
			len_tb[lft2] = 0;
			DbgInfo("BHT MSG:over boundary case\n");
			goto next;
		}
...
next:
	...
	DbgInfo("BHT MSG:tg sela:%xh, selb:%x\n", tga, tgb);
	DbgInfo("BHT MSG:rf sela:%xh, selb:%x\n", *rfa, *rfb);
	return TRUE;
}
...
static int ggc_get_first_0_index(u32 val)
{
	...
	DbgInfo("BHT MSG:oops-not find 0 index\n");
	return 0;
}
...
static int _tx_selb_inject_policy(int tx_selb, int org_selb)
{
	if ((org_selb & BIT_PASS_MASK) != BIT_PASS_MASK) {
		...
		case 2:
			...
			break;
		default:
			DbgInfo("BHT MSG:>= 3 point case\n");
		}
		org_selb &= tx_selb;
	}

	DbgInfo("BHT MSG:will check continous 0bits: 0x%x\n", org_selb);
	if (1)
	{
...
void tx_selb_inject_policy(struct sdhci_host *host, int tx_selb)
{
	...
	DbgInfo("BHT MSG:before inject, failed ragen 0x%x, tx_selb 0x%x\n", msm_host->ggc.ggc_cmd_tx_selb_failed_range, tx_selb);
	msm_host->ggc.ggc_cmd_tx_selb_failed_range = _tx_selb_inject_policy(tx_selb, msm_host->ggc.ggc_cmd_tx_selb_failed_range);
	tx_selb_failed_history_update(host, msm_host->ggc.ggc_cmd_tx_selb_failed_range);
	DbgInfo("BHT MSG:after inject %xh range:%xh\n", tx_selb,
		 msm_host->ggc.ggc_cmd_tx_selb_failed_range);
	if (is_bus_mode_sdr104(host))
		msm_host->ggc.sdr104.fail_phase = msm_host->ggc.ggc_cmd_tx_selb_failed_range;
	else
		msm_host->ggc.sdr50.fail_phase = msm_host->ggc.ggc_cmd_tx_selb_failed_range;
}
...
int get_selb_failure_point(int start, u64 raw_tx_selb, int tuning_cnt)
{
	...
	DbgInfo("BHT MSG:%s start:%d tuning_cnt:%d\n", __FUNCTION__, start,
			 tuning_cnt);
...
exit:
	DbgInfo("BHT MSG:%s: after adjust raw_tx_selb: 0x%llx, vct 0x%x\n",
    ...
    else
    {
        ...
        else
        {
            ...
            if(shif_left_flg)
            {
                ...
                DbgInfo("BHT MSG:shift left index\n");
            }
        }
...
static int _ggc_get_suitable_selb_for_next_tuning(struct sdhci_host *host)
{
	...
	if (selx_failure_point_exist(msm_host->ggc.ggc_cmd_tx_selb_failed_range)) {
		selb = msm_host->ggc.ggc_sw_selb_tuning_first_selb;
	} else {
		u32 inj_tx_selb = BIT_PASS_MASK;
		DbgInfo("BHT MSG:manual inject for all pass case\n");
		...
		DbgInfo("BHT MSG:manual inject for all pass case, inj_tx_selb=0x%x\n", inj_tx_selb);
		selb = get_best_window_phase(inj_tx_selb, NULL);
		DbgInfo("BHT MSG:select selb %d for all pass case\n", selb);
	}
	return selb;
}
...
void _ggc_reset_tuning_result_for_dll(struct sdhci_host * host)
{
	...
	msm_host->ggc.ggc_cmd_tx_selb_failed_range = BIT_PASS_MASK;
	msm_host->ggc.selx_tuning_done_flag = 0;
	_ggc_reset_sela_tuning_result(msm_host);
}
...
void ggc_dll_voltage_init(struct sdhci_host *  host)
{
	...
	for(i = 0; i < 4; i++)
	{
		msm_host->ggc.dll_voltage_scan_map[i] = 0;

	}
	msm_host->ggc.sdr50.dll_voltage_unlock_cnt = g_dll_voltage_unlock_cnt_100m;
	msm_host->ggc.sdr104.dll_voltage_unlock_cnt = g_dll_voltage_unlock_cnt_200m;
}
...
bool ggc_hw_inject_ext(struct sdhci_host * host, bool *card_status, u32 sel200, u32 sel100, bool writetobh201)
{
	...
	u8 data[512];
	t_gg_reg_strt gg_reg_arr[10];
	DbgInfo("BHT MSG:%s sel200:%xh,sel100:%xh\n", __FUNCTION__, sel200, sel100);
	...
	else
	{
		if (1)
		{
			u32 i = 0;
			u32 reg;
			DbgInfo("BHT MSG:%s: dump config data instead write to bh201\n", __FUNCTION__);
			for (i = 0; i < 128; i++)
			{
				memcpy(&reg, data+i*sizeof(u32), sizeof(u32));
				DbgInfo("BHT MSG:    ggc_reg32[%03d]=0x%08x\n", i, reg);
...
{
	{
		...
		struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
		struct sdhci_msm_host *msm_host = pltfm_host->priv;
		ret = ggc_hw_inject_ext(host, &card_status, msm_host->ggc.ggc_cmd_tx_selb_failed_range, msm_host->ggc.ggc_cmd_tx_selb_failed_range, writetobh201);
		DbgInfo("BHT MSG:ret:%x\n", ret);
		if((FALSE == ret) && (FALSE == card_status))
		{
			int selb =	BIT_PASS_MASK;
			DbgInfo("BHT MSG:inject again when hw inject\n");
			selb &= ~ GENERATE_IDX_VALUE(msm_host->ggc.ggc_sw_selb_tuning_first_selb);
			tx_selb_inject_policy(host, selb);
			_ggc_update_cur_setting_for_sw_selb_tuning(host, msm_host->ggc.ggc_cmd_tx_selb_failed_range); //update cur

			if(((11 - get_bit_number(msm_host->ggc.ggc_cmd_tx_selb_failed_range)) >= 5))
			{
			...
			if(max_recur--)
			{
				return _ggc_hw_inject_may_recursive(host, msm_host->ggc.ggc_cmd_tx_selb_failed_range, msm_host->ggc.ggc_cmd_tx_selb_failed_range, max_recur, writetobh201);
			}
			else
				return FALSE;
		}
			       ...
			       u64* raw_tx_selb)
{
	...
	if(TRUE == read_ret)
	{
		...
		selb_status_ggc_high32 = gg_reg_arr[3].value;
		log_bin(selb_status_ggc_high32);
		DbgInfo("BHT MSG:dll  sela after mask=%xh", gg_reg_arr[4].value);
		DbgInfo("BHT MSG:dll  selb after mask=%xh", gg_reg_arr[5].value);
	...
	if(card_status && read_status)
		DbgInfo("BHT MSG:card_status,read_status:%x %x\n", *card_status, *read_status);
	return ret;
}
	...
	if(FALSE == card_status) {
		if(TRUE == read_status) {
			...
			if (cmd7_timeout_issue_test_flag)
			{
				...
				DbgInfo("BHT MSG:inject selb %03x for force CMD7 read timeout error\n", selb);
				cmd7_timeout_issue_test_flag = FALSE;
			}
#else
			DbgInfo("BHT MSG:inject selb %03x for CMD7 read timeout\n", selb);
#endif
			tx_selb_inject_policy(host, selb);
		} else {
			DbgInfo("BHT MSG:read status failedA!!\n");
			DbgInfo("BHT MSG:============ %s dll:%xh failed ============\n", __FUNCTION__, cur_sela);
		}
		ret = FALSE;
		goto exit;
	} else {
		...
		if(TRUE == read_status) {
			...
			{
				DbgInfo("BHT MSG:============ %s dll:%xh need retuning ============\n", __FUNCTION__, cur_sela);
				psela_tuning_result[cur_sela] = RETUNING_CASE;
			}
			else
			{
				DbgInfo("BHT MSG:============ %s dll:%xh pass ============\n", __FUNCTION__, cur_sela);
				psela_tuning_result[cur_sela] = OUTPUT_PASS_TYPE;
			}
		}
		else
		{
			DbgInfo("BHT MSG:read status failed!!\n");
...
#define GGC_SW_SELB_TUNING_LOOP 3
	...
	//sel_test:
	//init
	msm_host->ggc.driver_strength_reinit_flg = 0;
	msm_host->ggc.dll_unlock_reinit_flg = 0;
	...
	if (ggc->tuning_cmd7_timeout_reinit_flg) {
		...
		dll_sela_basis = msm_host->ggc.ggc_cur_sela;
		cur_selb = msm_host->ggc.ggc_sw_selb_tuning_first_selb;
		DbgInfo("BHT MSG:Tuning will start from sela: 0x%x, selb: 0x%x where CMD7 timeout occur\n", dll_sela_basis, cur_selb);
	}
#else
	ggc->tuning_cmd7_timeout_reinit_flg = 0;
#endif
	...
	for (dll_sela_cnt = 0; dll_sela_cnt < TUNING_PHASE_SIZE; dll_sela_cnt++) {
		...
		DbgInfo("BHT MSG:============ %s select sela dll: %x, selb dll: %x ============\n", __FUNCTION__, cur_sela, cur_selb);
		if((NO_TUNING != psela_tuning_result[cur_sela]))
		{
			DbgInfo("BHT MSG:sela tuning=%d already tuning,so skip it\n", cur_sela);
			continue;
		}
rescan_selb:
		host_cmddat_line_reset(host);
		...
		if (dll_sela_cnt == 0) {
			...
			if (!!!selx_failure_point_exist(msm_host->ggc.ggc_cmd_tx_selb_failed_range)) {
				...
				DbgInfo("BHT MSG:no need rescan case\n");
			}
			status_ret = FALSE;
...
#ifdef FORCE_FIRST_CMD19_FAIL_ERROR
			{
				...
				{
					DbgInfo("BHT MSG:force first cmd19 fail to debug dll voltage change, cur_dll_voltage_idx=%d", ggc->cur_dll_voltage_idx);
			...
			if (FALSE == first_cmd19_sta) {
				...
				msm_host->ggc.dll_unlock_reinit_flg = 1; //change dll voltage
			...
			if (ret == FALSE || FALSE == card_status) {
				...
				if (FALSE == card_status) {
					...
					if (((11 - get_bit_number(ggc->ggc_cmd_tx_selb_failed_range)) >= 5))
					{
						...
						if (current_ds < 7)
							msm_host->ggc.driver_strength_reinit_flg = current_ds + 1;
						else
							msm_host->ggc.driver_strength_reinit_flg = 7;

						g_gg_reg_cur[15] &= 0x0F0FFFFF;
						g_gg_reg_cur[15] |= (msm_host->ggc.driver_strength_reinit_flg << 28)
															 | (msm_host->ggc.driver_strength_reinit_flg << 20);
						ret = FALSE;
						DbgErr("BHT ERR:will change driver strength from %d to %d\n",
									 current_ds,
									 msm_host->ggc.driver_strength_reinit_flg);
						goto exit;
					}
...
#ifdef FORCE_DRIVER_STRENGTH_ERROR
		{
			...
			while (driver_strength_reinit_test_flag && get_bit_number(msm_host->ggc.ggc_cmd_tx_selb_failed_range) > 6) {
		...
		if(FALSE == ret) {
			...
			msm_host->ggc.tuning_cmd7_timeout_reinit_flg = 1;
			...
			if((11 - get_bit_number(msm_host->ggc.ggc_cmd_tx_selb_failed_range)) >= 5) {
				...
				msm_host->ggc.tuning_cmd7_timeout_reinit_flg = 0;
		...
		DbgInfo("BHT MSG:===ot  sela:%xh pass ===\n", cur_sela);
		rescan_selb_cnt++;
		if ((rescan_selb_cnt < GGC_SW_SELB_TUNING_LOOP) &&
				(selx_failure_point_exist(msm_host->ggc.ggc_cmd_tx_selb_failed_range))) {
			DbgInfo("BHT MSG:rescan cnt %d, ggc_cmd_tx_selb_failed_range=0x%x\n",
							 rescan_selb_cnt,
							 msm_host->ggc.ggc_cmd_tx_selb_failed_range);
			goto rescan_selb;
		}
...
next_dll_sela:
		if ((FALSE == is_bus_mode_sdr104(host))
			...
			DbgInfo("BHT MSG:sdr50_notuning_sela_rx_inject:%x\n",
	...
	//
	{
...
#if  1
			DbgInfo("BHT MSG:inject sw sela failed point to all_selb\n");
			if (is_bus_mode_sdr104(host)) {
				cfg = 0x7ff;
			} else {
				cfg = 0x7ff;
		...
		// calculate selb
		{
...

			{
				...
				DbgInfo("BHT MSG:######### tx selb[%xh] 11 bit: %s \n",
								 tx_selb, win_tb);
			...
			}
			{
				...
				if (host->info.sw_cur_setting.sd_access_mode ==
					...
					// move for host
					DbgInfo("BHT MSG:base sela:%xh, selb:%x\n",
...
final:
	{
		...
		if (1) {
			...
			msm_host->ggc.selx_tuning_done_flag = 1;
		}

	}

exit:
	DbgInfo("BHT MSG:exit:%s  %d\n", __FUNCTION__, ret);
	return ret;
}
...
void ggc_tuning_result_reset(struct sdhci_host *host)
{
	...
	DbgInfo("BHT MSG:%s: will clear all tuning results\n", __FUNCTION__);
	_ggc_reset_tuning_result_for_dll(host);

	msm_host->ggc.sdr50.bus_mode = SD_FNC_AM_SDR50;
	msm_host->ggc.sdr104.bus_mode = SD_FNC_AM_SDR104;
	msm_host->ggc.driver_strength_reinit_flg = 0;
	msm_host->ggc.cur_bus_mode = NULL;
	msm_host->ggc.dll_unlock_reinit_flg = 0;
	msm_host->ggc.dll_unlock_reinit_flg = 0;
	msm_host->ggc.tuning_cmd7_timeout_reinit_flg = 0;
	msm_host->ggc.tuning_cmd7_timeout_reinit_cnt = 0;
	msm_host->sdr50_notuning_sela_inject_flag = 1;
	msm_host->sdr50_notuning_crc_error_flag = 0;
	msm_host->sdr50_notuning_sela_rx_inject = 0x0ff;
}
...
int sdhci_bht_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	if (sdhci_bht_target_host(host)) {	//wangpengpeng@wind-mobi.com modify for sd at 20180223
		...
		struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
		struct sdhci_msm_host *msm_host = pltfm_host->priv;
		DbgInfo("BHT MSG:enter bht tuning\n");

		if (host->clock < CORE_FREQ_100MHZ) {
			DbgInfo("BHT MSG:%d less 100Mhz,no tuning\n", host->clock);
			return 0;
		}

		if (msm_host->tuning_in_progress) {
			DbgInfo("BHT MSG:tuning_in_progress\n");
			return 0;
		}
		msm_host->tuning_in_progress = true;

		if (msm_host->ggc.selx_tuning_done_flag) {
			DbgInfo("BHT MSG:GGC tuning is done, just do msm host tuning");
			if (is_bus_mode_sdr104(host)) {
				ret = sdhci_bht_sdr104_execute_tuning(host, 0x13);
			} else {
				ret = sdhci_bht_sdr50_execute_tuning(host, 0x13);
