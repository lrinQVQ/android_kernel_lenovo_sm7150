...
// bayhub chevron.li add for degrade code at 2019/8/30 start
	...
	if(card->sd_bus_speed < 3){
			DbgInfo("%s: the next degrade will be SD2.0 current mode is 0x%08x\n",
							mmc_hostname(card->host),card->sd_bus_speed);
			card->host->v18_disable = 1;
	}

	DbgInfo("%s: the new mode is 0x%08x\n",
							mmc_hostname(card->host),card->sd_bus_speed);
#endif
...
void bht_load(struct mmc_host *mmc_host, struct mmc_card *card)
{
	...
	if ( sdhci_bht_target_host(host))//wangpengpeng@wind-mobi.com modify for sd at 20180223
	{
		...
		DbgInfo("%s: Load BHT patch RC8.3.1-JD20-DS=4\n", mmc_hostname(mmc_host));
		...
		if (msm_host->ggc.dll_unlock_reinit_flg) {
			int cur_dll_voltage_idx = msm_host->ggc.cur_dll_voltage_idx;
			DbgInfo("update dll voltage cfg for dll unlock reinit flg: idx=%d\n", cur_dll_voltage_idx);
		...
		if (msm_host->ggc.driver_strength_reinit_flg) {
			u8 driver_strength_reinit_flg = msm_host->ggc.driver_strength_reinit_flg;
			DbgInfo("%s: driver strength should be init to %d\n", mmc_hostname(mmc_host), driver_strength_reinit_flg);
			ggc_tuning_result_reset(host);
			if (msm_host->ggc.driver_strength_reinit_flg <= 7) {
		...
		if (msm_host->ggc.tuning_cmd7_timeout_reinit_flg == 0 && msm_host->ggc.selx_tuning_done_flag == 0) {
			DbgInfo("gg_sw_def[8] = 0x%08x, gg_sw_def[9]=0x%08x, gg_sw_def[15]=%08x\n", gg_sw_def[8], gg_sw_def[9], gg_sw_def[15]);
			...
			if (msm_host->ggc.selx_tuning_done_flag)
				DbgInfo("%s: skip load default configuration for tuning done\n", mmc_hostname(mmc_host));
			if (msm_host->ggc.tuning_cmd7_timeout_reinit_flg)
			{
				u8 data[512];
				DbgInfo("%s: write previous inject results to bh201 for cmd7 timeout flag is set\n", mmc_hostname(mmc_host));
				get_gg_reg_cur_val(data);
				if (1)
				{
					u32 i = 0;
					u32 reg;
					DbgInfo("%s: dump config data before write to bh201\n", __FUNCTION__);
					for (i = 0; i < 128; i++)
					{
						memcpy(&reg, data+i*sizeof(u32), sizeof(u32));
						DbgInfo("    ggc_reg32[%03d]=0x%08x\n", i, reg);
					}
				}
...
// bayhub chevron.li add for degrade code at 2019/8/30 start
	...
	DbgInfo("### Bayhub debug: degrade count is %d ###\n", host->degrade_count);
#endif
// bayhub chevron.li add for degrade code at 2019/8/30 end
	/*
...
/* huaqin add for SD card bringup by liufurong at 20190201 start */
#ifndef CONFIG_MMC_SDHCI_MSM_BH201
		/*
		 * Switch to wider bus (if supported).
		 */
		if ((host->caps & MMC_CAP_4_BIT_DATA) &&
...
// bayhub chevron.li add for degrade code at 2019/8/30 start
		...
		DbgInfo("### Bayhub debug: detect card removed ###\n");
		host->v18_disable = 0;
		host->legacy_mode = 0;
...
/* huaqin add for SD card bringup by liufurong at 20190201 start */
#ifdef CONFIG_MMC_SDHCI_MSM_BH201
retry:	if (sdhci_bht_target_host(mmc_priv(host))) {
		int retries = 5;
		while (retries) {
			...
			else
			{
				if(bretry == true)
				{
					bretry = false;
					host->degrade = 0;
					DbgInfo("card init degrade successfully!\n");
				}
			}
		...
		if (!retries) {
			...
			if(host->cur_sd_bus_speed >2){
				bretry = true;
				host->degrade = 1;
				DbgInfo("card init try to degrade\n");
				goto retry;
			}
			else
			{
