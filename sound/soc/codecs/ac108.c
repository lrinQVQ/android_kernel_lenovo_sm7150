...
#define AC108_CODEC_RW_TEST_EN	1  //0
#define AC108_ADC_PATTERN_SEL	ADC_PTN_0x5A5A5A		//0:ADC normal,  1:0x5A5A5A,  2:0x123456,  3:0x000000,  4~7:I2S_RX_DATA,  other:reserved


//AC108 config
#define AC108_CHANNELS_MAX		4  //6		//range[1, 16] 
...
//#define AC108_DVCC_NAME			"ac108_dvcc_1v8"
//#define AC108_AVCC_VCCIO_NAME	"ac108_avcc_vccio_3v3"
...
static void ac108_hw_init(struct i2c_client *i2c)
{
 ac108_write(0x00, 0x12, i2c);
 msleep(20);
 ac108_write(0x06, 0x01, i2c);
 ac108_write(0x07, 0x9b, i2c);
 ac108_write(0x09, 0x81, i2c);
 ac108_write(0x10, 0x4f, i2c);
 ac108_write(0x12, 0x00, i2c);
 ac108_write(0x13, 0xa0, i2c);
 ac108_write(0x14, 0x09, i2c);
 ac108_write(0x16, 0x0f, i2c);
 ac108_write(0x17, 0xd0, i2c);
 ac108_write(0x18, 0x01, i2c);
 ac108_write(0x20, 0x99, i2c);
 ac108_write(0x21, 0x93, i2c);
 ac108_write(0x22, 0x93, i2c);
 ac108_write(0x30, 0x15, i2c);
 ac108_write(0x31, 0x00, i2c);
 ac108_write(0x32, 0x00, i2c);
 ac108_write(0x33, 0x0f, i2c);
 ac108_write(0x34, 0x1d, i2c);
 ac108_write(0x35, 0x33, i2c);
 ac108_write(0x36, 0x60, i2c);
 ac108_write(0x38, 0x01, i2c);
 ac108_write(0x39, 0x03, i2c);
 ac108_write(0x3C, 0x08, i2c);
 ac108_write(0x60, 0x08, i2c);
 ac108_write(0x61, 0x1f, i2c);
 ac108_write(0x66, 0x0f, i2c);
 ac108_write(0x90, 0x00, i2c);
 ac108_write(0x91, 0x00, i2c);
 ac108_write(0x92, 0x00, i2c);
 ac108_write(0x93, 0x00, i2c);
 ac108_write(0xA0, 0x07, i2c);
 ac108_write(0xA7, 0x07, i2c);
 ac108_write(0xAE, 0x07, i2c);
 ac108_write(0xB4, 0x07, i2c);
 ac108_write(0xB5, 0x07, i2c);
 ac108_write(0xBB, 0x0F, i2c);
 
}
void ac108_start(void)
{
	ac108_hw_init( i2c_clt[0]);	/*0x01=0x80: VREF Enable*/
	pr_err("[AC108] %s\n", __func__);
}

void ac108_stop(void)
{
	ac108_write(CHIP_AUDIO_RST, 0x12, i2c_clt[0]);	/*0x01=0x80: VREF Enable*/
	pr_err("[AC108] %s\n", __func__);
}
EXPORT_SYMBOL_GPL(ac108_start);
EXPORT_SYMBOL_GPL(ac108_stop);
...
static int ac108_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	for(i=0; i<ARRAY_SIZE(ac108_sample_rate); i++){
		if(ac108_sample_rate[i].real_val == params_rate(params) / (AC108_ENCODING_EN ? AC108_ENCODING_CH_NUMS/2 : 1)){
			...
			ac108_set_pll(dai, 1, 0, 1536000, 24576000);
...
#endif
        if (regulator_en && !IS_ERR(ac108->vol_supply.dvcc_1v8) && !IS_ERR(ac108->vol_supply.avcc_vccio_3v3)) {
		ret = regulator_disable(ac108->vol_supply.avcc_vccio_3v3);
		if(ret != 0)
		        pr_err("[AC108] %s: some error happen, fail to disable regulator 3v3!\n", __func__);
		ret = regulator_disable(ac108->vol_supply.dvcc_1v8);
		if(ret != 0)
		        pr_err("[AC108] %s: some error happen, fail to disable regulator 1v8!\n", __func__);
		regulator_en = 0;
	}
...
static int ac108_resume(struct snd_soc_codec *codec)
{
	...
	if (!regulator_en && !IS_ERR(ac108->vol_supply.vcc3v3)) {
        ...
        if (!regulator_en && !IS_ERR(ac108->vol_supply.dvcc_1v8) && !IS_ERR(ac108->vol_supply.avcc_vccio_3v3)) {
		ret = regulator_enable(ac108->vol_supply.dvcc_1v8);
		if(ret != 0)
			pr_err("[AC108] %s: some error happen, fail to enable regulator 1v8!\n", __func__);
		ret = regulator_enable(ac108->vol_supply.avcc_vccio_3v3);
		if(ret != 0)
			pr_err("[AC108] %s: some error happen, fail to enable regulator 3v3!\n", __func__);
		regulator_en = 1;
	}
...
#endif
        if (!regulator_en){
                ac108->vol_supply.dvcc_1v8 = regulator_get(NULL, "pm6150_l10");
	        if (IS_ERR(ac108->vol_supply.dvcc_1v8) || !ac108->vol_supply.dvcc_1v8) {
		        pr_err("get ac108 dvcc_1v8 failed, return!\n");
		        return -EFAULT;
	        }
	        regulator_set_voltage(ac108->vol_supply.dvcc_1v8, 1800000, 1800000);
	        ret = regulator_enable(ac108->vol_supply.dvcc_1v8);
	        if(ret != 0)
		        pr_err("[AC108] %s: some error happen, fail to enable regulator dvcc_1v8!\n", __func__);

	        pr_debug("%s:enable regulator dvcc_1v8 successful!", __func__);
                ac108->vol_supply.avcc_vccio_3v3= regulator_get(NULL, "pm6150_l16");
	        if (IS_ERR(ac108->vol_supply.avcc_vccio_3v3) || !ac108->vol_supply.avcc_vccio_3v3) {
	                pr_err("get ac108 avcc_vccio_3v3 failed, return!\n");
		        return -EFAULT;
		}
	        regulator_set_voltage(ac108->vol_supply.avcc_vccio_3v3, 3300000, 3300000);
	        ret = regulator_enable(ac108->vol_supply.avcc_vccio_3v3);
	        if(ret != 0)
		        pr_err("[AC108] %s: some error happen, fail to enable regulator avcc_vccio_3v3!\n", __func__);
	
	        pr_debug("%s:enable regulator avcc_vccio_3v3 successful!", __func__);
		regulator_en = 1;
        }
#if 0
...
const struct dev_pm_ops ac108_pm_ops = {
	.resume=ac108_i2c_resume,
	.suspend=ac108_i2c_suspend,
};
